...
from .univla_noise_schedulers import FlowMatchingScheduler
...
class Emu3RotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = (
            torch.arange(self.max_seq_len_cached, device="cpu")
            .float()
...
class Emu3MoE(Emu3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.action_experts:
            ...
            self.action_projector = ActionProjector(
                config.action_dim, action_config.hidden_size
            )
            ...
            self.action_decoder = FinalLayer(
                action_config.hidden_size, config.action_dim
            )
    ...
    def forward(
        ...
        if action is not None and self.action_experts and self.training:
            ...
            velo_pred, hidden_states_refine = self.forward_action(
                noise_action, tau, hidden_states
            )
    ...
    def forward_action(self, z, t, cond):
        ...
        for action_layer in self.action_layers:
            action_hidden_states = action_layer(action_hidden_states)[0]
        hidden_states, action_hidden_states = (
            action_hidden_states[:, :seq_len, :],
            action_hidden_states[:, seq_len:, :],
        )
    ...
    def generate_action(self, outputs, sample_steps=20, frames=8, action_dim=7):
        ...
        # action generation
        z = torch.randn((batch_size, frames, action_dim), dtype=hidden_states.dtype).to(
